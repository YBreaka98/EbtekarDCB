            ...
            <div class="mx-auto flex w-full max-w-md flex-col space-y-16">
                <div class="flex flex-col items-center justify-center text-center space-y-8">
                    ...
                    <div class="font-semibold text-3xl">
                        <p>أدخل رمز التحقق لتغيير الإشتراك الحالي</p>
                    </div>
                </div>

                <div>
                    <form id="confirmUpgradeForm" method="post">
                        <div class="flex flex-col space-y-16">
                            ...
                            <div class="flex flex-col space-y-5">
                                ...
                                <div class="mt-1 text-center">
                                    <a href="{{ route('subscription-list', $msisdn) }}" class="text-sm text-blue-500 hover:text-blue-700 focus:outline-none focus:underline">العودة إلى صفحة الإشتراكات</a>
                                </div>
                            </div>
                        </div>
                    </form>
...
@push('scripts')
    ...
    <script>
        ...
        function confirm(event) {
            ...
            let msisdn = window.location.pathname.split('/')[2];
           ...
           $.ajax({
                ...
                error: function (xhr, status, error) {
                    ...
                    if (JSON.parse(xhr.responseJSON?.error).messageCode == '01') {
                        Errors.postMessage(JSON.stringify('رمز التحقق غير صحيح'));
                    } else {
                        Errors.postMessage(JSON.stringify('حدث خطأ أثناء تسجيل الدخول. الرجاء المحاولة مرة أخرى'));
                    }
                }
            });
        }
    ...
    <script>
        ...
        document.getElementById('confirmUpgradeForm').addEventListener('submit', confirm);
    </script>
